...
namespace Weather
{
    ...
    public class Wind : MonoBehaviour
    {
        ...
        public static Vector3 WindStuff()
        {
            ...
            windDirection.x = Mathf.Max(localWindVector.x, WindDirTendancy(Latitude, orbitingBody).x, WindGust.x);
            windDirection.y = Mathf.Max(localWindVector.y, WindDirTendancy(Latitude, orbitingBody).y, WindGust.y);
            windDirection.z = Mathf.Max(localWindVector.z, WindDirTendancy(Latitude, orbitingBody).z, WindGust.z); //Don't want the overlay texture to move upwards...
        ...
        public Vector3 WindGusts(float windSpeed, Vector3 windDirection)
        {
            ...
            if(isWindGust == true)
            {
                ...
                float numberz = UnityEngine.Random.Range(-20, 20);
        ...
        public static Vector3 ChangeWindVector(Vector3 initialVector, Vector3 targetVector, float deltaTime)
        {
            initialVector = Vector3.Lerp(initialVector, targetVector, deltaTime);

            return initialVector;
        }

        public static float ChangeWindSpeed(float initialSpeed, float targetSpeed, float deltaTime)
        {
            while(initialSpeed < targetSpeed)
            {
                initialSpeed = Mathf.Lerp(initialSpeed, targetSpeed, deltaTime);
            }
            return initialSpeed;
        }
