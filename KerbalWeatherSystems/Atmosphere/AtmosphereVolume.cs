...
{
    public class AtmosphereVolumeMaterial : KWSMaterialManager
    {
        [Persistent]
    ...
    class AtmosphereVolume
    {
        ...
        public bool Scaled
        {
            get { return AtmosphereMesh.layer == KWSManagerClass.SCALED_LAYER; }
            set
            {
                if (value)
                {
                    float scale = (float)(1000f / celestialBody.Radius);
                    Reassign(KWSManagerClass.SCALED_LAYER, scaledCelestialTransform, scale);
                }
                else
                {
                    Reassign(KWSManagerClass.MACRO_LAYER, celestialBody.transform, 1);
                }
            }
        }
        ...
        private static Shader AtmosphereShader
        {
            get
            {
                if (atmosphereShader == null)
                {
                    ...
                    atmosphereShader = KWSManagerClass.GetShader(assembly, "Atmosphere.Shaders.Compiled-SphereAtmosphere.shader");
